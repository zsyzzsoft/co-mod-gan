...
def training_loop(
    ...
    misc.save_image_grid(grid_reals, dnnlib.make_run_dir_path('reals.png'), drange=training_set.dynamic_range, grid_size=grid_size, pix2pix=training_set.pix2pix)
    ...
    for gpu in range(num_gpus):
        def auto_gpu(opr):
            if opr.type in ['ExtractImagePatches', 'SparseSlice']:
                return '/cpu:0'
            else:
                return '/gpu:%d' % gpu
        ...
        with tf.name_scope('GPU%d' % gpu), tf.device(auto_gpu):
            ...
            with tf.name_scope('DataFetch'):
                ...
                masks_var = tf.Variable(name='masks', trainable=False, initial_value=tf.zeros([sched.minibatch_gpu, 1] + training_set.shape[1:]))
                ...
                masks_write = training_set.get_random_masks_tf()
                ...
                masks_read = masks_var[:minibatch_gpu_in]
    ...
    while cur_nimg < total_kimg * 1000:
        ...
        if cur_tick < 0 or cur_nimg >= tick_start_nimg + sched.tick_kimg * 1000 or done:
            ...
            if image_snapshot_ticks is not None and (cur_tick % image_snapshot_ticks == 0 or done):
                grid_fakes = Gs.run(grid_latents, grid_labels, grid_reals, grid_masks, minibatch_size=sched.minibatch_gpu)
                misc.save_image_grid(grid_fakes, dnnlib.make_run_dir_path('fakes%06d.png' % (cur_nimg // 1000)), drange=drange_net, grid_size=grid_size, pix2pix=training_set.pix2pix)
